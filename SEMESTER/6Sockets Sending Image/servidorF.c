...
int main(int argc, char *argv[])
{
  ...
  int socket_id;
  int id_canal;
  struct sockaddr_in servidor;//estructura que contiene la ip y el puerto
  char sendBuff[1025];
  int numrv;
  ...
  if(bind(socket_id, (struct sockaddr*)&servidor,sizeof(servidor))==-1)//hace al socket disponible
    {close(socket_id);perror("Bind");exit(1);}
  
  if(listen(socket_id, BACKLOG) == -1){perror("Listen\n");exit(1);}//empieza a escuchar clientes en el socket
