...
void * productor(void * argv){ // 2 - 1
  ...
  for(int p=0;p<5;p++){
    ...
    //usleep(100000);
      waitS(1); //wait al productor    //    no puede pasar nadie hasta que el hilo escoja que zona quiere
...
void * consumidor(void *argv){
  for(int p=0;p<5;p++){
    ...
    //usleep(100000);
    waitS(2);//wait a si mismo, consumidor
    for(int o=0;o<2;o++){
      if(zonacritica[o][0]=='e'){
        ...
        zonacritica[o][0]='v';
        signalS(0);//ya vacie uno, le sumo al primer contador, banquero libre
        break; }
...
//semaforo0 contador, semaforo1 binario productor, semaforo2 binario consumidor, semaforo3 contador?
  if((semctl(Semcb,2,SETVAL,2)||semctl(Semcb,1,SETVAL,1)||semctl(Semcb,2,SETVAL,0)||semctl(Semcb,3,SETVAL,2))==-1){
